...
// Today, we will look on how to use the 1.8″ ST7735  colored TFT display with Arduino. The past few tutorials have been focused on how to use the Nokia 5110 LCD display extensively
// but there will be a time when we will need to use a colored display or something bigger with additional features, that’s where the 1.8″ ST7735 TFT display comes in.
...
int analogP1 = 14;
int analogP2 = 15;
int grinderActivationPin = 16;
int grindStatusPin = 17 ;
int timeOrWeightPin = 18 ;
...
void setup() {
  ...
  pinMode(grinderActivationPin, OUTPUT);
  pinMode(grindStatusPin,INPUT);
  pinMode(analogP1,INPUT);
